...
class Talk(object):
    ...
    def sendTagLocation(self, to, text, contentMetadata={}):
        ...
        msg.contentType, msg.contentMetadata = 15, contentMetadata
        location = Location()
        location.address = "ֆɛʟʄɮօȶ"
        location.phone = None
        location.latitude = 25.01
        location.longitude = 122.55
        location.title = "ֆɛʟʄɮօȶ"
        msg.location = location
        if to not in self._messageReq:
            self._messageReq[to] = -1
        self._messageReq[to] += 1
    ...
    def sendContact(self, to, mid):
        ...
        res = self.sendMessage(to, '', contentMetadata, 13)
        self.mid_tracking[mid] = self.newMetaByContact(res.id, to, self.profile.mid)
        return res
    ...
    def addFriendByMid(self, mid, tracking = None):
        if tracking == None:
            self.sendContact(self.profile.mid, mid)
    ...
    def getGroupIdsByName(self, groupName):
        gIds = []
        for gId in self.getAllChatMids().memberChatMids:
            g = self.getChatV2(gId)
            if groupName in g.chatName:
                gIds.append(gId)
        return gIds
    @loggedIn
    def cloneContactProfile(self, mid):
        ...
        profile.pictureStatus = self.downloadFileURL(
            'http://dl.profile.line-cdn.net/' + contact.pictureStatus, 'path')
